...
vi.mock("./pipeline/intentExtractor", () => ({
  ...
  detectRequirementChange: vi.fn(async () => ({
  ...
  mergeRequirements: vi.fn((current: any, changes: any) => ({ ...current, ...changes })),
  formatChangeConfirmation: vi.fn(() => "✅ Параметры обновлены"),
  defaultRequirements: vi.fn((topic: string) => ({
    topic,
    slideCount: null,
    enableImages: null,
    userWillAddImages: false,
