...
3. Для редизайна (redesign_context) — используй ТОЛЬКО шаги S3 → S4 → S5.
   Нарратив (S1, S2) уже есть в existing_results, менять его НЕ нужно.
   S3 должен создать НОВУЮ дизайн-систему по запросу пользователя.
   S4 должен перегенерировать слайды с новым дизайном, сохраняя контент.
   S5 должен проверить качество.
4. Каждый шаг должен иметь уникальный step_id (начиная с 1).
...
Пользователь хочет сменить стиль существующей презентации. Нарратив и структура
слайдов уже готовы (результаты S1 и S2 доступны в existing_results).

Твоя задача — составить план ТОЛЬКО из шагов S3 → S4 → S5:
- S3: Создать НОВУЮ дизайн-систему по запросу пользователя.
- S4: Перегенерировать слайды с новым дизайном, сохраняя контент.
- S5: Проверить качество.

Запрос на новый стиль: {style_request}
"""
...
def _build_user_prompt(store: SharedStore) -> str:
    ...
    # Контекст редизайна
    redesign_context = store.user_input.get("redesign_context")
    if redesign_context:
        parts.append(
            REDESIGN_CONTEXT_ADDITION.format(
                style_request=redesign_context.get("style_request", ""),
            )
        )
