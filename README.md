...
|---|---|---|---|
| 1 | **Research Agent** | 3% | Собирает факты, статистику и данные через веб-поиск до планирования структуры |
| 2 | **Analysis Agent** | 5% | Кластеризует исследования, выделяет якорные инсайты, строит нарративную дугу |
| 3 | **Planner** | 8% | Анализирует тему с учётом анализа, определяет язык, генерирует заголовок и branding-контекст |
| 4 | **Outline Agent** | 12% | Создаёт структуру на основе реальных данных: заголовки, content_shape, ключевые тезисы |
| 5 | **Outline Critic** | 18% | Оценивает структуру + покрытие исследовательских данных по 10-балльной шкале |
| 6 | **Writer** (параллельный) | 30% | Генерирует контент с учётом research + analysis highlights |
| 7 | **Storytelling Agent** | 40% | Улучшает нарратив с учётом нарративной дуги из анализа |
| 8 | **Theme Selector** | 48% | AI-подбор визуальной темы на основе анализа промпта (тональность, отрасль, аудитория) |
| 9 | **Layout Mapper** | 55% | Маппинг content_shape → HTML-макет с учётом разнообразия и плотности контента |
| 10 | **Image Generator** | 60% | Выбирает слайды для иллюстраций и генерирует AI-изображения |
| 11 | **Speaker Coach** | 71% | Генерирует заметки спикера для каждого слайда с таймингами |
| 12 | **Data Viz Agent** | 72% | Анализирует данные и генерирует SVG-графики (bar, line, pie, donut, radar, horizontal-bar) |
| 13 | **HTML Composer** (LLM) | 75% | Маппит контент Writer → данные для HTML-шаблонов через LLM |
| 14 | **QA Agent** | 85% | Структурная валидация: required fields, типы данных, иконки, лимиты. Auto-fix (`fixSlideStructure`) |
| 15 | **Design Critic** | 92% | Визуальная валидация: контраст, плотность, overflow, баланс. Auto-fix (`fixSlideDensity`) |
| 16 | **Assembly** | 95% | Финальная сборка: рендеринг HTML через Template Engine, инъекция CSS-темы, нумерация слайдов |
...
- **PPTX** — экспорт в PowerPoint через pptxgenjs (поддержка 50 типов слайдов)
- **PDF** — экспорт в PDF
...
|---|---|---|
| Pipeline (агенты) | ~18 800 | 26 |
| Routes (API) | ~10 800 | 22 |
| Server (прочее) | ~3 500 | 13 |
| Client (страницы) | ~7 500 | 9 |
