...
AI Presentation Generator — это full-stack веб-приложение, которое превращает текстовое описание темы в полноценную HTML-презентацию. Система использует **10 специализированных AI-агентов**, работающих последовательно и параллельно, для создания структуры, контента, дизайна и визуализации данных.
...
| AI-агенты | 10 специализированных агентов |
...
| `scenario_cards` | Сценарии | scenario-cards, card-grid |
...
- Мультиагентный AI-пайплайн с 10 специализированными агентами
...
- **PPTX** — экспорт в PowerPoint через pptxgenjs (поддержка 15+ типов слайдов)
- **PDF** — экспорт в PDF
...
| Nunjucks | Шаблонизация HTML-слайдов |
| Jose | JWT-токены |
...
Приложение использует **37+ REST API endpoints** через Express. Основные группы:
...
| GET | `/:id/progress` | SSE-стриминг прогресса генерации |
...
| POST | `/sessions/:id/files` | Загрузить файлы |

### Редактирование слайдов (`/api/v1/slides`)

| Метод | Endpoint | Описание |
|---|---|---|
| PUT | `/:presentationId/:slideIndex` | Обновить слайд |
| POST | `/:presentationId/:slideIndex/regenerate` | Регенерировать слайд |
| PUT | `/:presentationId/reorder` | Переупорядочить слайды |
| GET | `/:presentationId/:slideIndex/versions` | История версий слайда |
| POST | `/:presentationId/:slideIndex/revert` | Откатить к версии |
...
**REST vs tRPC.** Приложение использует Express REST API для основной функциональности (SSE streaming, бинарные экспорты, file uploads, публичные endpoints) и tRPC для аутентификации. Решение обосновано тем, что 27 из 37 endpoints требуют Express-специфичные возможности. Новые endpoints рекомендуется добавлять через tRPC.
